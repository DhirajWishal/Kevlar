...
public class Connector {
    ...
    /**
     * This function is the same as the earlier function BUT the difference here it doesn't send the encoded database but instead sends the validation key to the....
     * ...server as the server doesn't contain the validation key
     *
     * @param userName      User's Username
     * @param password      User's Password
     * @param validationKey User's Validation Key
     ...
     * @throws IOException              make sure no I/O error has occurred
       */
    private String newUserDataToXML(String userName, String password, String validationKey, String ivData) throws IOException {
        String encodedDatabase = base64TheFile();
        String userDataXML = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
    ...
    /**
     * This function will update the SERVER's database on any password changes or username changes, called when server data wants to be updated
     *
     * @param userName      User's Username
     * @param password      User's Password
     * @param validationKey User's Validation Key
    ...
    /**
     * This function will check if the account already exists in the server, the data is sent to the server and the server compares its data within itself.
     * ..amd returns a Status code
    ...
    /**
     * This function will fetch the data from the server and get the Username and all the details and assign them to variables and then pass them to a userAccount object
     * and return it as a userAccount object
     *
     * @param userName      User's Username
     * @param password      User's Password
     * @param validationKey User's Validation Key
     * @return the Object userAccunt
     * @throws IOException              make sure no I/O error has occurred
     * @throws NoSuchAlgorithmException makes sure the requested data is available
     * @throws InvalidKeyException      makes sure the key is in the correct format, length etc..
     */
    public UserAccount getUserAccount(String userName, String password, String validationKey) throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        ...
        for (int temp = 0; temp < kevlarDataByNode.getLength(); temp++) {
            ...
            if (kevlarNode.getNodeType() == Node.ELEMENT_NODE) {
                Element kevlarElement = (Element) kevlarNode;
                /**
                 * serverUserData gets the data stored in tag <username>
                 * serverPassword gets the data stored in tag <password>
                 * serverDatabase gets the data stored in tag <database>
                 * serverHMac gets the data stored in tag <hmac>
                 * serverIV gets the data stored in tag <iv>
                 */
